...
A support inbox contains only issues. Over time, staff begin to believe that issues are increasing, even when overall volume and error rates are stable or improving. An increase in near-miss reporting is interpreted as worsening safety, rather than improved transparency. Senior leaders hear only what reaches escalation — by the time information reaches them, it is already filtered for urgency, risk, and failure.

When busyness is mistaken for crisis, organisations over-invest in fixing things that are not broken while genuine problems elsewhere go unnoticed.
...
Teams understand both the failure rate and the success rate. Incident counts are always paired with total volume. System health is discussed separately from incident handling. People can distinguish between a system that is struggling and one that is simply visible only when it fails.
